...
## 벤치마킹
성능 비교는 **JMH(Java Microbenchmark Harness)**를 이용해 진행했으며 수동 매핑 방식을 기준으로 비교한다.
...
`MethodMutator`의 내부를 보면 다음 코드가 존재하는데, 원본 객체의 값을 대상 객체의 프로퍼티에 할당할 때, 대상 객체의 set 메서드를 실행해서 값을 매핑한다.
